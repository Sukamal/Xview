...
public class PieChartView extends View {
    ...
    private ArrayList<PieChartHelper> pieHelperList;
    ...
    private Runnable animator = new Runnable() {
        ...
        public void run() {
            ...
            for(PieChartHelper pie : pieHelperList){
                pie.update();
                if(!pie.isAtRest()){
                    needNewFrame = true;
                }
    ...
    public PieChartView(Context context, AttributeSet attrs){
        ...
        pieHelperList = new ArrayList<PieChartHelper>();
        cirPaint = new Paint();
        ...
        textPaint.setTextSize(Utility.sp2px(getContext(), 13));
    ...
    public void setDate(ArrayList<PieChartHelper> helperList){
        initPies(helperList);
        pieHelperList.clear();
        removeSelectedPie();

        if(helperList != null && !helperList.isEmpty()){
            for(PieChartHelper pieHelper:helperList){
                pieHelperList.add(new PieChartHelper(pieHelper.getStartDegree(),pieHelper.getStartDegree(),pieHelper));
            }
        }else {
            pieHelperList.clear();
        }
...
// postInvalidate();
     ...
     * @param helperList
     */
    private void initPies(ArrayList<PieChartHelper> helperList){
        float totalAngel = 270;
        for(PieChartHelper pie:helperList){
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int index = 0;
        for(PieChartHelper pieHelper : pieHelperList){
    ...
    private void drawPercentText(Canvas canvas, PieChartHelper pieHelper){
    ...
    private void drawText(Canvas canvas, PieChartHelper pieHelper){
